...
{   
    ...
    if (p0p1.cross(p0p).z() > 0) return p1p2.cross(p1p).z() > 0 && p2p0.cross(p2p).z() > 0;
    else return p1p2.cross(p1p).z() < 0 && p2p0.cross(p2p).z() < 0;
}

static std::tuple<float, float, float> computeBarycentric2D(float x, float y, const Vector3f* v)// 平滑插值, 计算权重
{
...
void rst::rasterizer::rasterize_triangle(const Triangle& t) {
    ...
    int x_min = floor(MIN(MIN(v[0].x(), v[1].x()), v[2].x()));
    int y_min = floor(MIN(MIN(v[0].y(), v[1].y()), v[2].y()));
    int x_max = ceil(MAX(MAX(v[0].x(), v[1].x()), v[2].x()));
    int y_max = ceil(MAX(MAX(v[0].y(), v[1].y()), v[2].y()));

    auto& frame_box = rst::rasterizer::frame_buffer();
