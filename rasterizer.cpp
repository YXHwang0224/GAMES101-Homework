...
auto to_vec4(const Eigen::Vector3f& v3, float w = 1.0f) {//将一个三维矩阵变为点形式的四维矩阵
    return Vector4f(v3.x(), v3.y(), v3.z(), w);
}

static bool insideTriangle(int x, int y, const Vector4f* _v) {//判断是否在三角形内
    Vector3f v[3];
    for (int i = 0; i < 3; i++)
...
static std::tuple<float, float, float> computeBarycentric2D(float x, float y, const Vector4f* v) {//返回重心坐标
...
void rst::rasterizer::draw(std::vector<Triangle*>& TriangleList) {
    ...
    Eigen::Matrix4f mvp = projection * view * model;//MVP变化
    for (const auto& t : TriangleList) {//对每一个三角形进行遍历
        Triangle newtri = *t;

        // 取得三角形在三维空间下的原始坐标 (还未投影到平面)
        ...
        Eigen::Vector4f v[] = {//v中储存被遍历的三角形经过MVP变换后的坐标
                mvp * t->v[0],
                mvp * t->v[1],
                mvp * t->v[2]
        };
...
interpolate(float alpha, float beta, float gamma, const Eigen::Vector3f& vert1, const Eigen::Vector3f& vert2,//二维插值函数
    const Eigen::Vector3f& vert3, float weight) {
    return (alpha * vert1 + beta * vert2 + gamma * vert3) / weight;
}

static Eigen::Vector2f
interpolate(float alpha, float beta, float gamma, const Eigen::Vector2f& vert1, const Eigen::Vector2f& vert2,//三维插值函数
...
void rst::rasterizer::clear(rst::Buffers buff) {//清空buffer中的信息
