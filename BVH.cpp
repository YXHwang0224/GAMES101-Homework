...
Intersection BVHAccel::getIntersection(BVHBuildNode* node, const Ray& ray) const
{
    ...
    if (node->left == nullptr && node->right == nullptr) {
        Intersection res_intersection;
        res_intersection = node->object->getIntersection(ray);
        return res_intersection;
    }
    Intersection left_intersection, right_intersection;
    left_intersection = getIntersection(node->left, ray);
    right_intersection = getIntersection(node->right, ray);
