...
//    - 当射线在外面时。
//
// 如果光线在外面，则需要使 cosi 为正 cosi = -N.I
//
// 如果光线在内部，则需要反转折射率并抵消法线 N
// [/comment]
...
// [comment]
// 如果光线与对象相交，则返回 true，否则返回 false。
//
// \param orig 是光线原点
// \param dir 是光线方向
// \param objects 是场景包含的对象列表
// \param[out] tNear 包含到最接近的相交对象的距离。
// \param[out] index 存储相交三角形的索引（如果相交的对象是网格）。
// \param[out] uv 存储相交点的 u 和 v 重心坐标
// \param[out] *hitObject 存储指向相交对象的指针（用于检索材料信息等）
// \param isShadowRay 是阴影光线. 一旦找到命中，我们可以更快地从函数返回。
// [/comment]
std::optional<hit_payload> trace(
        const Vector3f &orig, const Vector3f &dir,
        const std::vector<std::unique_ptr<Object> > &objects)
{
    float tNear = kInfinity;//初识将tNear设置为kINfinity，kINfinity为极大值
    std::optional<hit_payload> payload;//payload为碰撞信息
    for (const auto & object : objects)//对每个物体进行判定
    {
        float tNearK = kInfinity;
        uint32_t indexK;
        Vector2f uvK;
        // 条件 tNearK < tNear 找到离观测点最近的物体
        // intersect函数是判断光线是否与该物体相交，并记录该物体与光线原点的距离至tNeark，不同的物体intersect函数实现不一样
...
// [comment]
// 主 render 函数。在这里，我们迭代图像中的所有像素，生成
// Primary rays 并将这些光线投射到场景中。framebuffer 的内容是
// 保存到文件中。
// [/comment]
void Renderer::Render(const Scene& scene)
{
    //framebuffer为像素合集（vector保存）
